...
seeds_input = input[0].split(' ').drop(1).map(&:to_i)
map_input = input.drop 1

seeds = []
seeds_input.each_slice(2) do |range_start, range_length|
  (range_start..range_start + range_length).each do |seed|
    seeds << seed
  end
end
...
# seems like the data sets are too large to go this route.  instead, i'll need to
# figure out how to do this using ranges vs seeds themselves.
